...
const (
	...
	BUSY_GROUP                                = "BUSYGROUP Consumer Group name already exists"
	NO_GROUP                                  = "NOGROUP No such key"
)
...
func (c *Consumer) Ack(ctx context.Context, messageId string) error {
	...
	log.Println("Acknowledging message successfully", messageId)
...
func (c *Consumer) validateError(ctx context.Context, err error) error {
	if strings.Contains(err.Error(), NO_GROUP) {
